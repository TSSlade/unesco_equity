...
#datapath <- "D:/Users/ccampton/Documents/unesco_equity/data/SHRPUganda/2. 2018 Uganda SHRP C1 (Baseline FU1 FU2 FU3 FU4 FU5 ENDLINE) S.dta"
...
xlsxCols <- c("dataset",	"measure_label","grade", "treat_phase","gini","mean", "cv", "ratio_p90p10","ratio_p75p25", "pct_zero")
...
for(sp in 1:nsubpops){
  ...
  for( p in 1:nphases){
    ...
    UgandaXlsx[(sp-1)*nphases+p+1,] <- c('Uganda', 'English', mode(xlsxphase$grade), mode(xlsxphase$treat_phase), Gini(SESphase), mean(SESphase), sd(SESphase)/mean(SESphase), quantile(SESphase,.90)/zero2na(quantile(SESphase,.10)), quantile(SESphase,.75)/zero2na(quantile(SESphase,.25)), sum(SESphase==0)/length(SESphase))
    }
    }
...
write.xlsx(x = UgandaXlsx, file = "inequality_results_Uganda.xlsx",sheetName = "core", row.names = FALSE, col.names=FALSE)
