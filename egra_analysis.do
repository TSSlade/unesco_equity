...
// Sanity check
groups grade treat_phase sub_pops
...
label define lbl_sub_population 1 "base_c1_con_gr_1"  2 "base_c1_con_gr_2" 3 "base_c2_con_gr_1"  4 "base_c2_con_gr_2" 5 "base_c3_con_gr_1"  6 "base_c3_con_gr_2" 7 "mid_c1_trt_gr_1" 8 "mid_c1_trt_gr_2" 9 "mid_c2_con_gr_1"   10 "mid_c2_con_gr_2" 11 "mid_c3_con_gr_1"  12 "mid_c3_con_gr_2" 13 "end_c1_trt_gr_1" 14 "end_c1_trt_gr_2" 15 "end_c2_trt_gr_1" 16 "end_c2_trt_gr_2" 17 "end_c3_con_gr_1" 18 "end_c3_con_gr_2" , replace      // Let's make some labels
...
local dataset "primr"
foreach l of num 1/`n_lang_vars' {                              // Looping over Eng, Kis
    di "Inside the languages"
    di "Sub_pop ct: `n_sub_pops' in outer file"
    foreach s of num 1/`n_sub_pops' {                           // Looping over grade, treatment phase
        di "Inside the sub_pops"
        foreach c of num 1/`n_cluster_levels' {                 // Looping over clustering at school level (or not)
            di "Inside the clusters"
            local clustering `: word `c' of `cluster_levels''   // Human-readable clustering status
            di "Cluster status: [`clustering']"
            * pause
            * if `c'== 1 {
            *     local lang_var `: word `l' of `clustered_lang_vars''    // School-level clustering --> schl_eng_orf | schl_kis_orf
            *     local zero_var `: word `l' of `zero_vars''              // School-level clustering --> schl_zero_pct_eng | schl_zero_pct_kis
            * }
            * else {
                local lang_var `: word `l' of `lang_vars''              // No clustering --> e_orf_a | k_orf
                local zero_var `: word `l' of `zero_vars''              // No clustering --> e_orf_a | k_orf
            * }
            ...
            local lang_name `: word `l' of `lang_names''                // HUman-readable (eng, kis)
            di as error "++++++++++++++++++++++++++++++++++++++"
            di as result "Now writing results for l=[`l'], lang_name=[`lang_name'], lang_var=[`lang_var']"
            di as error "++++++++++++++++++++++++++++++++++++++"
...
export excel "bins/inequality_calculations.xlsx", sh("$c_datetime") firstrow(var) update


quietly apply_pshare eq_orf k_eq_orf, data("PRIMR") sub(sub_population) var("English Kiswahili") res("inequality_results") splabel(`splabs')