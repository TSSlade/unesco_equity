...
for (i in 1:length(subpopFeats)){
  ...
  for (j in 1:length(uniqueFeat)){
    ...
    colnames(temp) <- SESfeat
    SESind <- projPC(temp)
    binnedLine(SESind,SES$orf.end-SES$orf.base,10,paste0("Change in ORF verse SES index for grade ",uniqueFeat[j]), "SES index","Change in ORF")
...
write.xlsx(x = ntileTab, file = paste0(xlsxpath,"SES_primr.xlsx"),sheetName = paste0(ntile,"-percentiles"), row.names = FALSE, col.names=FALSE)
