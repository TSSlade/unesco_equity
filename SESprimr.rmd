...
for(sp in 1:nsubpops){
  ...
  for(spv in 1:length(subpopVars)){
    masktemp <- masktemp & (as.character(unlist(dataTemporal[,subpopVars[spv]]))==substr(subpops[sp],spv,spv))
  }
...
ntileTab <- data.frame(matrix(ncol=(2*nsubpops+2),nrow=(4*(ntile+1)+2)))
