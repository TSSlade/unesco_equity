...
tabcnames <-  c("Dataset","Language","Grade","Phase","Mean","CV","p90/p10","p75/p25","% zero")
...
for (i in 1:length(dsets)){
  ...
  for (j in 1:length(langs)){
    ...
    for (k in 1:length(gr)){
      Dsub <- D[D$dataset == dsets[i] & D$measure_label == langs[j] & D$grade ==gr[k],c("treat_phase","mean", "cv", "ratio_p90p10", "ratio_p75p25","pct_zero")]
      nk = nrow(Dsub)
      summaryTab[loci+locj + lock + nk, "Grade"] <- "______"
      summaryTab[loci+locj + lock + ceiling(nk/2), "Grade"] <- gr[k]
      summaryTab[(loci+locj+lock+1):(loci+locj+lock+nk),c("Phase","Mean","CV","p90/p10","p75/p25","% zero")] <- Dsub
      lock = lock +nk
    }
    locj = locj +numj
  }
...
summaryTab[,c("Mean","CV","p90/p10","p75/p25","% zero")] <- lapply(summaryTab[,c("Mean","CV","p90/p10","p75/p25","% zero")],as.numeric)
summaryTab[,c("Mean","CV","p90/p10","p75/p25","% zero")] <- lapply(summaryTab[,c("Mean","CV","p90/p10","p75/p25","% zero")],round,3)
